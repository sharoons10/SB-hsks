    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`http://localhost:5000/carts/${item._id}`).then(response => {
          if (response) {
            refetch();
             Swal.fire("Removed!", "Your booking has been removed.", "success");
           }
        })
        .catch(error => {
          console.error(error);
        ...
        <div className="py-28 flex flex-col items-center justify-center">
          ...
          <div className=" text-center px-4 space-y-7">
            <h2 className="md:text-5xl text-4xl font-bold md:leading-snug leading-snug">
              Services Added to The<span className="text-green"> Cart</span>
            </h2>
          </div>
        </div>
      </div>
...

      {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start my-12 gap-8">
          ...
          <div className="md:w-1/2 space-y-3">
            ...
            <p>Total Services: {cart.length}</p>
            <p>
              Total Price:{" "}
        ...
        <p>Cart is empty. Please add services.</p>
