...
const Cards = ({ item }) => {
  ...
  const handleAddToCart = item => {
    ...
    if(user && user.email){
          ...
          if(response){
            refetch(); // refetch cart
              Swal.fire({
                  position: 'center',
                  icon: 'success',
                  title: 'Service added to the cart.',
        ...
        Swal.fire({
            title: 'Please login to book the services',
            icon: 'warning',
      ...
      <div className="card-body">
       <Link to={`/menu/${item._id}`}><h2 className="card-title">{item.name}</h2></Link>
        ...
        <div className="card-actions justify-between items-center mt-2">
          ...
          <button onClick={() => handleAddToCart(item)} className="btn bg-green text-white">Book Service </button>
        </div>
      </div>
    </div>
  );
